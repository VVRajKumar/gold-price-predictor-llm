...
st.caption(
    "All price predictions are generated by a **stacked ML ensemble** "
    "(XGBoost + LightGBM + Ridge). Confidence bands come from quantile "
    "regression. The LLM writes narrative only — it never predicts prices."
)
...
if _shap:
    ...
    with col_model:
        ...
        <h4>Model Stack</h4>
        <p><b>{_shap.get('model_type', 'ML Ensemble')}</b></p>
        <p>Features: {_shap.get('total_features', '?')}</p>
        <p>Bands: Quantile Regression (80% interval)</p>
        <p>Corrections: Residual EWMA bias learning</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_shap:
        ...
        if _feat_imp:
            ...
            fig_shap = px.bar(
                ...
                orientation="h",
                title="SHAP Feature Importance (what drives the prediction)",
                color="importance",
                color_continuous_scale=["#2d3748", "#00d4aa"],
            )
    ...
    # Per-hour SHAP drivers
    _hourly_shap = _shap.get("hourly_drivers", [])
    if _hourly_shap:
        with st.expander("🔍 Per-Hour SHAP Drivers (what moves each hour)"):
            for hd in _hourly_shap[:12]:
                st.markdown(
                    f"**Hour {hd['hour']}:** {' · '.join(hd.get('drivers', []))}"
                )
else:
    st.info("SHAP explainability will appear after the first ML prediction cycle.")
